#!/usr/bin/env python3
"""
CPU-only fine-tune DialoGPT with LoRA on Solana DeFi datasets
"""
...
# Configuration
MODEL_NAME = "microsoft/DialoGPT-small"  # Using smallest model for CPU
...
def fine_tune_llama():
    """Fine-tune DialoGPT with LoRA on Solana DeFi dataset"""
    
    logger.info("Starting DialoGPT fine-tuning with LoRA (CPU only)...")
    ...
    tokenized_dataset = solana_dataset.map(tokenize_function, batched=True)
    ...
    training_args = TrainingArguments(
        ...
        warmup_steps=5,
        max_steps=20,  # Very small for CPU demo
        learning_rate=5e-4,
        logging_steps=1,
        save_steps=10,
        save_total_limit=2,
